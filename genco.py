    ...
    if bus in selected_pmus:
        ...
        for i, f in enumerate(features):
            ...
            if i in range(0, 3):  # per unit voltage
                ss_data_temp[:, :, i] = ss_data_temp[:, :, i] * bus_data[f][b_index] / base_voltage
            elif i in range(3, 6):  # per unit current
                ss_data_temp[:, :, i] = ss_data_temp[:, :, i] * bus_data[f][b_index] / base_current
            elif i in range(6, 9):  # per unit current
                ss_data_temp[:, :, i] = np.cos(ss_data_temp[:, :, i] * bus_data[f][b_index] * np.pi / 180)
