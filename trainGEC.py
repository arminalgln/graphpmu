...
def train_model(model, graph_model, train_dataset, val_dataset, pmu_number, n_epochs):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for bat_num, batch_input in enumerate(train_dataset):
            ...
            # graph update
            final_latent = enocdergraph(batch_input.to(device))
            # if bat_num == 0:
            #     print('first')
            #     all_final = final_latent
            # else:
            #     print(bat_num)
            #     all_final = torch.cat((all_final,final_latent))
        ...
        print('encpar_before', next(iter(enocdergraph.encoder.parameters()))[0,0])
        print('gpar_before', next(iter(enocdergraph.graph.parameters()))[0,0])
        endGEC_optimizer.step()
        print('mu_aa',mu[0,0])
        print('encpar_a', next(iter(enocdergraph.encoder.parameters()))[0,0])
        print('gpar_a', next(iter(enocdergraph.graph.parameters()))[0,0])
        # endGEC_optimizer.zero_grad()
...
model, graph_model = train_model(
    ...
    pmu_number=pmu_number, n_epochs=10
)
